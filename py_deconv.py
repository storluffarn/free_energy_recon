...
# The problem is in the binning process. 
...
def get_signal(x, support, bins, z_min, z_max):
    ...
    for k in range(traces):
        ...
        for i in range(1,samples):
            ext_work[k,i] = ext_work[k,i-1] - 0.5*K*( (x[i,k]-support[i]) + (x[i-1,k]- support[i-1]) )*(support[i] - support[i-1])      # w = F*x
...
def summed_maxima(x,z,range_tuple):
    ...
    for i in range(n_periods):
        ...
        S +=max(z[t])   # ok, I admit, it's cool that python can do this, find a maximum in some subilst z[t] of the list z, where t is itself a list
    return S
...
def periodic_bc (i,bins) :
    k = (i - int(0.5*(bins+1))) % bins
    return k
...
def make_kernel(M,N,beta,K,bin_centers,lc_bins):    
    ...
    #3print lc_bins[a]
    #print M/2-1
    #print bin_centers[M/2-1]
    #print lc_bins[0], lc_bins[1]
    #print bin_centers[0]
    #print lc_bins[len(lc_bins)-2], lc_bins[len(lc_bins)-1]
    #print bin_centers[len(bin_centers)-1]

    #print bin_centers[0] - lc_bins[0]
    a = []
    for i in range(M) :
        a.append(bin_centers[i] - lc_bins[i])
    a.append(abs(bin_centers[M-1]-lc_bins[M]))

    savetxt('signals/ecdist.dat',a)
    savetxt('signals/lc_bins.dat',lc_bins)
    savetxt('signals/bin_centers.dat',bin_centers)
    ...
    for j in range(N) :
        works[j] = exp(-0.5*beta*K*( (bin_centers[M/2] - lc_bins[j])**2 ))

    savetxt('signals/works.dat',works)
...
#   This is the oroginal kernel that's slightly wrong in the ends
#def make_kernel(M,N,beta,K, bin_centers, lc_bins):
#    kernel = zeros((M,N))
#    norm_vector = zeros(N)
#    for i in range(M):
...
# this is important, it IS the deconvolution
def LR_deconv(signal, extra_bins, bin_centers,K, beta, ITER):
    ...
    # cut = (N-M)/2               # guaranteed integer.   # well, yes, that's just extra_bins?
    # this is important, this is the O and I definitions from the papers
    O = ones(N)*sum(signal)
    I = H.dot(O)
    O_old = O
    ...
    count = 0;
    while count<ITER:       # This is the famous stoping criterion?
...
def get_signal_bi(x_f, x_r, support, bins, z_min, z_max, traces, samples):
    ...
    ext_work_r = ext_work_f*0
    exp_work_r = exp_work_f*0
        ...
        for k in range(traces):
            ...
            for l in range(bins):
                if (x[i,k] > bin_edges[l] and x[i,k] <= bin_edges[l+1]):
                    #print "Hit!"
                    hist[l,i] += exp(-beta*ext_work[k,i])/traces # normalizing on every term, for some reason and why is it += ?!
