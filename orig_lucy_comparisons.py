...
from orig_py_utils import *
from orig_py_deconv import *
...
from orig_local_lucy import local_lucy,lucy_max,lucy_summed_max, local_lucy2, local_lucy3
...
system("./origsimu/run -M {} -Z {} -E {} -N {} -d signals -t {}".format(runs,Z, E,2*T,T))
...
#x2.plot(z/sigma,beta*g_bi, color="red", label=r"$g^{LR}_{bi}$")
...
ax2.set_ylabel(r"$\Delta F_{surf} [k_{B}T]$",size=25)
...
fig.suptitle(r"Different stoppers for LR-Deconv")
plt.show()
