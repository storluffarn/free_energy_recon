// why does it say that h_i is set but unused
...
void wham_reconstruct_interp_rare(double ** x_ar, double * lambda_ar,
...
void berkovic_works(double ** ext_work, double ** x_ar, 
		double *lambda_ar, int samples, int traces,
...
void get_third_cumulant(double ** ext_work, int samples, int traces,
			double * weight_work, double beta);
void make_protocol(int reverse, double * prot, double T_final,
	   	double z_final, double * lambda, 
		double h, int steps);
...
void ermak(double * x, double * p, double lambda1, 
	   double lambda2, double x_rand, double p_rand,
...
void wham_reconstruct(double ** x_ar, double * lambda_ar,
			  ...
			  double * G, double epsilon, double sigma, double * U_surf, 
			  double * recon_slope, double * recon_msd, double * work_mean, 
			  double * work_std, double * exp2_avarage,
		      double * avarage_work, double * rms, 
			  int * invalid_count, double * bin_centers,
		      double * sample_work_variances,double * G_interp, 
			  double * U_interp, double * bin_interp,
...
void lc_deconvolution(double ** x_ar, double * lambda_ar,
		      int traces, int samples, double K, double beta, int recon_samples,
		      double z_start, double z_final, double epsilon, 
			  double sigma, double * U_lc_surf, double * work_mean, 
...
void get_work_dist(double ** x_ar, double * lambda_ar,
		   int traces, int samples, double K,
		   double * work_vector);
