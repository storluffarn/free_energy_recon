...
double force(double x, double lambda, double beta, double D,
  ...
  return -beta*D*(epsilon*(2*M_PI/sigma)*sin(2*M_PI*x/sigma)
		  +K*( x - lambda));
}
// double well force...
/* double force(double x, double lambda, double beta, double D, */
...
/* 		  ); */
/* } */
double ermak_force(double x, double lambda, double beta, double D,
		   double epsilon, double sigma, double K){
...
void ricci(double * x, double * p, double lambda1, double lambda2, double K, double W,
  ...
  x_temp = *x + t_m*e2*(*p) + t2_2m*ermak_force(*x, lambda1, beta, D, epsilon, sigma, K) + x_rand;
  /* now x(t+dt) is stored in x_temp */
  *p = e1*(*p) + h_2*e4*ermak_force(x_temp, lambda2, beta, D, epsilon, sigma, K) + h_2*ermak_force(*x, lambda1, beta, D, epsilon, sigma, K)*e34 + p_rand;
  *x = x_temp;
...
void ermak(double * x, double * p, double lambda1, double lambda2, double x_rand, double p_rand,
  ...
  double x_temp;
  x_temp = *x + (*p)*eta_coeff*m_i + t2_2m*ermak_force(*x, lambda1, beta, D, epsilon, sigma, K) + x_rand;
  *p = exp_eta*(*p) + eta_coeff2*(ermak_force(*x, lambda1, beta, D, epsilon, sigma, K)+ermak_force(x_temp, lambda2, beta, D, epsilon, sigma, K) + p_rand);
  *x = x_temp;
...
void RK4(double * x, double t, double * rk,
  ...
  rk[0] = rk_force(*x, *lambda, epsilon,K, sigma,
		   beta, rand,D );  
  calculate_lambda(lambda,t + 0.5*h ,T_final,z_final,A,B);
  rk[1] = rk_force(*x + 0.5*h*rk[0], *lambda, epsilon,K, sigma,
		   beta, rand,D );
  rk[2] = rk_force(*x + 0.5*h*rk[1], *lambda, epsilon,K, sigma,
		   beta, rand,D );
  calculate_lambda(lambda,t + h ,T_final,z_final,A,B);
